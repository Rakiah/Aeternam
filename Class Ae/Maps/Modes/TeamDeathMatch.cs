...
public class TeamDeathMatch : GamesModes
{
	public List<Team> Teams = new List<Team>();

	public TeamDeathMatch (int pointsPerKill, int defaultObjective, int maxObjective, int minObjective)
	{
		m_iObjectives = defaultObjective;
		m_iMaxObjectives = maxObjective;
		m_iMinObjectives = minObjective;
		
		m_sName = "Team Death Match";
	...
	public override void UpdateTeams (int TeamID, int PlayerID)
	{
		if(Network.isServer) 
		{ 
			if(!Teams[TeamID].available) return; 
			AeCore.m_pCoreGame.m_pNetworkHandler.networkView.RPC("UpdatePlayerTeam", RPCMode.AllBuffered, TeamID, PlayerID);    
		}
	...
	public override void RegisterKill (int Killer, int Killed, int weapon, int BodyPart)
	{
		...
		{ 
			...
			RegisterObjective(KillerStats.m_iTeamID);
		}
